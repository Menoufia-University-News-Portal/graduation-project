//  const id = Number(window.location.pathname.replace("/comersnews/", ""));
//  `http://localhost:3001/comers-news/${id}`
//  `http://localhost:3001/comers-news/update/${id}`
    ...
    const filteredFacultiesIds = Array.isArray(form.faculties) ? form.faculties.filter(id => id !== null) : [];
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const res = axios.patch(`http://localhost:3001/comers-news/update/${id}`, payload );
          console.log(res);
              //  if (res.data) {
