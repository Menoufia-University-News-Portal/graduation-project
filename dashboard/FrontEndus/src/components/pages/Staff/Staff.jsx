...
export default function Staff() {
    ...
    const [delete_staff, setdeletestaff] = useState(false);
    const [filters, setFilters] = useState({
        global: { value: null, matchMode: FilterMatchMode.CONTAINS },
        staff_id: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.EQUALS }] },
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const listOfstaff = response.data;
                setstaff(listOfstaff);

            } catch (error) {
                console.error('Error fetching data of staff:', error);
            }
        };
        fetchData();
    }, [delete_staff]);
    ...
    async function handledelete(id) {
        ...
        // try {
        //     const res = await axios.delete(`http://localhost:3001/staff/delete/${id}`)
        //     console.log(res);
        //     setdeletestaff((prev) => !prev);
        // } catch (err) {
        //     console.log(err);
    ...
    const ActionBodyTemplate = ({ data }) => {
        ...
        return (
            <div className='d-flex align-items-center gap-2'>
                <Link to={`${data.staff_id}`}>
                    {/* <FaPenToSquare style={{ fontSize: "25px", color: "green" }} /> */}
                    <button className="btn2"> Update
                    </button>
                </Link>
                <button className="btn3" onClick={() => handledelete(data.staff_id)} > Delete
                </button>
                {/* <FaTrash onClick={() => handledelete(data.staff_id)} style={{ fontSize: "25px", color: 'crimson', cursor: 'pointer' }} /> */}
            </div>
        );
    };
            ...
            <div>
                ...
                <div className="card">
                    <DataTable
                        ...
                        filters={filters}
                        filterDisplay="menu"
                        globalFilterFields={['staff_id', 'name']}
                    ...
                    >
                        ...
                        <Column
                            ...
                            sortable
                            filter
                            body={(data) => <RoleBodyTemplate data={data} />}
