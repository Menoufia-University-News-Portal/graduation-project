...
export default function SectorHeadNews() {
    const [sectorhead, setsectorhead] = useState([]);
    const [selectedEvents, setSelectedEvents] = useState([]);
    const [deletecomers, setdeletecomers] = useState(false);
    const [filters, setFilters] = useState({
        global: { value: null, matchMode: FilterMatchMode.CONTAINS },
        sector_head_news_id: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.EQUALS }] },
    ...
    useEffect(() => {
        const res = fetch('http://localhost:3001/sector-head-news/list')
            .then(res => res.json())
            .then((data) =>setsectorhead(data));
    ...
    const renderHeader = () => {
        return (
            ...
            <div className="first" >
                <h4 className="m-0"> Sector Head Page </h4>
    ...
    const IDBodyTemplate = ({ data }) => {
        ...
        return (
            <div className="flex align-items-center gap-2">
                <span>{data.sector_head_news_id}</span>
            </div>
        );
    };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    axios.delete(`http://localhost:3001/sector-head-news/delete/${id}`)
    ...
    const ActionBodyTemplate = ({ data }) => {
        ...
        return (
            <div className='d-flex align-items-center gap-2'>
                <Link to={`${data.sector_head_news_id}`}>
                ...
                <button onClick={() => handledelete(data.sector_head_news_id)} className="btn3"> Delete
                 </button>
                {/* <FaTrash onClick={() => handledelete(data.sector_head_news_id)} style={{ fontSize: "25px", color: 'crimson', cursor: 'pointer' }} /> */}
            </div>
        );
    };
          ...
          <path class="three" d="M0.154393339,3.89485454 L3.97631488,0.139296592 C4.17083111,-0.0518420739 4.48263286,-0.0518571125 4.67716753,0.139262789 L25.6916134,20.7848311 C26.0855801,21.1718824 26.0911863,21.8050225 25.704135,22.1989893 C25.7000188,22.2031791 25.6958657,22.2073326 25.6916762,22.2114492 L4.67709797,42.8607841 C4.48259567,43.0519059 4.17082418,43.0519358 3.97628526,42.8608513 L0.154518591,39.1069479 C-0.0424848215,38.9134427 -0.0453206733,38.5968729 0.148184538,38.3998695 C0.150289256,38.3977268 0.152413239,38.395603 0.154556228,38.3934985 L16.9937789,21.8567812 C17.1908028,21.6632968 17.193672,21.3467273 17.0001876,21.1497035 C16.9980647,21.1475418 16.9959223,21.1453995 16.9937605,21.1432767 L0.15452076,4.60825197 C-0.0425130651,4.41477773 -0.0453986756,4.09820839 0.148075568,3.90117456 C0.150162624,3.89904911 0.152268631,3.89694235 0.154393339,3.89485454 Z" fill="#FFFFFF"></path>
            ...
            <div className="card" >
                ...
                <DataTable
                    ...
                    dataKey="sector_head_news_id"
                    selectionMode="checkbox"
                    selection={selectedEvents}
                    onSelectionChange={(e) => setSelectedEvents(e.value)}
                    filters={filters}
                    filterDisplay="menu"
                    globalFilterFields={["sector_head_news_id", 'title']}
                    emptyMessage="No sector_head found."
                    currentPageReportTemplate="Showing {first} to {last} of {totalRecords} entries"
                >
                    <Column
                        field="sector_head_news_id"
                        header="ID"
                        sortable
                        dataType="numeric"
